...
func testFrost(tt *testing.T, suite frost.Suite) {
	...
	peers, keyShareCommits := dealer.Deal(rand.Reader, privKey)
...
func benchmarkFrost(b *testing.B, suite frost.Suite) {
	...
	dealer, err := frost.NewDealer(suite, t, n)
	test.CheckNoErr(b, err, "failed to create dealer")
	privKey := frost.GenerateKey(suite, rand.Reader)

	b.Run("DealKey", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			dealer.Deal(rand.Reader, privKey)
		}
	})
	...
	b.Run("KeyShareVerif", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
